...
def from_ora_to_csv_with_date(execution_date) :
    ...
    data = oracle_hook.get_pandas_df(sql="select ORA_HASH(table_name||tablespace_name) oid, s.* FROM serps.client_from_star s, all_tables WHERE owner='SERPS' AND TABLE_NAME = 'CLIENT_FROM_STAR'")
...
with DAG(
  dag_id="Al_from_ora_to_postgres", 
